...
class RoleAuthority extends Component {
  ...
  fetch(eid) {
    ...
    api.postDs("Account/RolePermissions?roleId=" + eid).then( (res) => {
      let rs = res.Data
  ...
  componentDidMount() {
    var eid = this.props.record["Id"]
    this.fetch(eid)
  }

  onOk() {
    api.postDs("Account/RolePermissionsUpdate?roleId=" + this.eid,this.selectedKeys).then((res) => {
      if (!res.Result) {
        alert(res.Message)
      }
      this.props.onCancel()
